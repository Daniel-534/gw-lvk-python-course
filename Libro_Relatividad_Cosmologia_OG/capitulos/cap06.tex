...
\begin{equation}
  g^{\rho\mu}(\nabla_\lambda R_{\rho\sigma\mu\nu} + \nabla_\rho R_{\sigma\lambda\mu\nu} + \nabla_\sigma R_{\lambda\rho\mu\nu}) = 0.
\end{equation}
Usando las simetrías del tensor de Riemann ($R_{\rho\sigma\mu\nu} = -R_{\sigma\rho\mu\nu}$, $R_{\rho\sigma\mu\nu} = -R_{\rho\sigma\nu\mu}$) y la definición del tensor de Ricci $R_{\sigma\nu} = g^{\rho\mu}R_{\rho\sigma\mu\nu}$, cada término se simplifica:
\begin{align}
...
Cuando Hubble descubrió en 1929 que el universo se está expandiendo (ley de Hubble-Lemaître), Einstein retiró $\Lambda$ lamentando haberla introducido. Según el relato del físico George Gamow, Einstein la calificó de su ``mayor equivocación'' (\textit{größter Fehler}).
